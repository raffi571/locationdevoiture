...
if ($res) {
    ...
    $columns = $connexion->get_all_columns_from_table("voiture");
    $num = $connexion->total_count_number("immatriculation", "voiture")[0]["COUNT(immatriculation)"];
    if (array_key_exists("page", $_GET) && array_key_exists("select", $_GET)) {
        $s = $_GET["page"] * $_GET["select"];
        $client = $connexion->find_data("voiture", $s, $_GET["select"]);
    }
}
...
<script>
    window.onload = function () {
        ...
        if (!select && !page) {
            window.location.href = "affichageVoitures.php?select=3&page=0";
        }

        if (!page && select) {
            window.location.href = "affichageVoitures.php?select=" + select + "&page=0";
        }
    ...
    function Set_Location(page) {
        let select = getQueryVariable("select");
        window.location.href = "affichageVoitures.php?select=" + select + "&page=" + page;
    }
    ...
    function next_page(max) {
        ...
        if (page_next <= max) {
            window.location.href = "affichageVoitures.php?select=" + select + "&page=" + page_next;
        }
...
<body>
    <header>
        ...
        <div class="menu-show" id="menu-show">
            ...
            <a href="affichageLocations.php">Affichages Location</a>
            <hr>
            <label>Formulaire</label>
            <hr>
    ...
    <main>
        <h3>Affiche les Voitures</h3>
        <form method="get" action="affichageVoitures.php">
